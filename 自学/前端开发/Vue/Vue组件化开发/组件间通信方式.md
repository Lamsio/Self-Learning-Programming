...
export default {  
 ...
 mounted() {  
    pubsub.subscribe('hello',function(){  
      console.log("Hands says: Someone publish hello msg.")  
    })  
 }  
}  
</script>
...
###### Pubsub-js 取消订阅
同样的，Pubsub-js也是需要取消订阅事件，但这与原生方式并不相同，当我们打算取消订阅时，我们需要使用`pubsub.unsubscribe([事件ID])`，这种设计类似于定时器，他并不是靠事件名判断，而是靠事件ID。每当我们执行`pubsub.subscribe()`时，都会返回一个唯一的订阅ID。