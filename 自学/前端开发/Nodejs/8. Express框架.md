...
其实原理就和**条件中间件**说的一样，当所有路由都匹配完后就会匹配`app.use()`中的中间件，在路由`*`中，我们尝试访问一个不存在的文件，那么就会报错，错误数据被传入`next`中，并传递给`err_handler()`中的`err`参数，值得注意的是错误中间件`err_handler`的`(err, req, res, next)`四个参数缺一不可！